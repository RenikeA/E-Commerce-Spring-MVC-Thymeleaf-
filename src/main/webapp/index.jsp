    ...
    <div class="container">
        ...
        <div class="header-action">
            ...
            <button class="header-action-btn" aria-label="user">
                ...
                <span class="btn-badge" id="person-badge">0</span>
            </button>
            ...
            <button class="header-action-btn" aria-label="cart">
                ...
                <span class="btn-badge" id="cart-badge">Name</span>
            </button>
                                ...
                                <ul class="card-action-list">
                                    ...
                                    <li>
                                        <button class="card-action-btn" aria-label="add to cart" title="add to cart">
                                            <ion-icon name="add-outline" aria-hidden="true" value="product1"></ion-icon>
                                        </button>
                                    </li>

                                    <li>
                                        <button class="card-action-btn" aria-label="add to cart" title="add to cart">
                                            <ion-icon name="bag-handle-outline" aria-hidden="true" value="product1"></ion-icon>
                                        </button>
                                    </li>

                                    <li>
                                        <button class="card-action-btn" aria-label="add to whishlist" title="add to whishlist">
                                            <ion-icon name="heart-outline" aria-hidden="true" value="product1"></ion-icon>
                                        </button>
                                    </li>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Select all buttons with the class 'card-action-btn'
        var buttons = document.querySelectorAll('.card-action-btn');

        // Add click event listener to each button
        buttons.forEach(function(button) {
            button.addEventListener('click', function(event) {
                // Check if the button has aria-label or title set to "add to whishlist"
                var ariaLabel = button.getAttribute('aria-label');
                var title = button.getAttribute('title');

                if (ariaLabel === 'add to whishlist' || title === 'add to whishlist') {
                    // Prevent default action (e.g., following a link)
                    event.preventDefault();

                    // Log the data being sent to the console
                    console.log('Sending data to UserController:');

                    // Get the relevant data from the button and its parent elements
                    var userID = 'yourUserID'; // Replace with the actual way to retrieve userID
                    var productID = button.querySelector('ion-icon').getAttribute('value');
                    var lastWishListUpdateUserID = 'admin';
                    var lastWishListUpdateDate = '2024-06-10';

                    // Log the data to the console
                    console.log('UserID:', userID);
                    console.log('ProductID:', productID);
                    console.log('LastWishListUpdateUserID:', lastWishListUpdateUserID);
                    console.log('LastWishListUpdateDate:', lastWishListUpdateDate);

                    // Construct form data
                    var formData = new FormData();
                    formData.append('action', 'wishlistAction');
                    formData.append('userID', userID);
                    formData.append('wishListProductIDs', productID);
                    formData.append('lastWishListUpdateUserID', lastWishListUpdateUserID);
                    formData.append('lastWishListUpdateDate', lastWishListUpdateDate);
